...
use crate::{Float, Body, constants::*};
...
fn gm() {
    ...
    let earth = Body::new(EARTH_MASS_KG, EARTH_RADIUS_KM, EARTH_RADIUS_KM);
...
#[test]
fn p_4_1() {
    let earth: Body = Body::new(EARTH_MASS_KG, EARTH_RADIUS_KM, EARTH_RADIUS_KM);
...
mod motions
{
    ...
    fn p_4_4() {
        let earth: Body = Body::new(EARTH_MASS_KG, EARTH_RADIUS_KM, EARTH_RADIUS_KM);
        let r_p: Float = earth.radius_equator_m() + 250_000.0;
        let r_a: Float = earth.radius_equator_m() + 500_000.0;
        let gm: Float = earth.gm();
    ...
    fn p_4_5() {
        ...
        let r_p: Float = earth.radius_equator_m() + alt_p; // Radius of orbit at periapsis in meters
        let denominator: Float = 2.0 * gm / (r_p * v_p.powi(2)) - 1.0;
        let r_a: Float = r_p / denominator;
        let alt_a: Float = r_a - earth.radius_equator_m();
